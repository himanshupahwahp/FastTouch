 ...
 <div class="card">
 	<article class="card-group-item">
 		...
 		<div class="filter-content">
 			<div class="card-body">
 			<form>';
  $sqlCategory = "SELECT DISTINCT(category) from post_details";
...
function displayAds($image, $post_no, $title, $desciprtion) {
  ...
  if(isset($_GET["category"]) && !empty($_GET["category"]))
  {
    ...
    $sql = "SELECT post_no, title, category, desciprtion, image FROM post_details where title LIKE '$search' AND category = '$category'";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "SELECT post_no, title, category, desciprtion, image FROM post_details where title LIKE '$search'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            // output data of each row
